...
> **ステータス:** 現在のバージョンは `0.1.1` で、npm で公開されています。
...
要件: Node.js `>=22.19.0`、Pi Coding Agent `>=0.84.0`。次のコマンドを実行して Pi-TUIX をインストールします。

```bash
pi install npm:pi-tuix --approve
```

Pi がパッケージを自動的にダウンロードして、次回以降のセッションで読み込みます。インストール後は Pi を終了して再起動してください。`pi list` で読み込み元を確認できます。

現在のセッションで `/pituix-default` を入力すると Pi 標準 UI に戻り、`/pituix` で Pi-TUIX を再び有効にできます。

### ローカル開発版をインストールする

ローカルソースをテストする場合は、先にリポジトリをクローンしてください。
...
ローカル開発版のインストール方法は上記のコマンドを使用します。
