...
要求：Node.js `>=22.19.0`、Pi Coding Agent `>=0.84.0`。执行下面一条命令安装 Pi-TUIX：

```bash
pi install npm:pi-tuix --approve
```

Pi 会自动下载并加载这个 TUI。安装完成后，退出并重新打开 Pi。使用下面的命令确认当前加载的包：

```bash
pi list
```

在当前会话中输入 `/pituix-default` 可以恢复 Pi 原生界面，输入 `/pituix` 可以重新启用 Pi-TUIX。
...
修改扩展代码后请重启 Pi。使用 `pi install -l /absolute/path/to/pi-tuix --approve` 可进行项目级安装，使用 `pi -e ./extensions/index.ts` 可进行不保存的一次性预览。
...
连续成功的 Read/Bash 调用会合并为数量摘要：文件路径去重，Bash 按调用次数计数。展开工具后可查看每一次调用。成功的 Read 读图也计入文件数量，展开后显示字节摘要。错误、取消、Bash 图片和截断结果单独显示；助手正文和其他工具会分隔摘要。恢复会话时通过 Pi 的公开会话分支重建分组。
...
普通和全屏终端模式都会按参考样式实时显示用户和助手消息，同时保留 Pi 的编辑器、流式输出、工具、通知、组件和队列。助手 Markdown 继续使用 Pi 的公开解析器和语法高亮，并按实测结果调整显示：隐藏代码围栏，让围栏代码与普通回答正文对齐，在 Pi 计算出的表格列宽内居中表头，并在引用竖线旁使用斜体正文。实时消息、`/pituix-transcript` 和恢复预览共用同一套处理。全屏模式的滚动、提示词跳转、搜索和鼠标选择由 Pi 的原生视口处理，共用同一份消息排版，关闭搜索后保留命中位置。普通模式使用终端滚动历史，也可以在 Pi 设置中直接切换两种模式。`/pituix-default` 可以在同一会话中恢复默认界面。
...
主对话和这两种预览会在用户消息旁显示 `[Image #1]` 等图片附件，包括仅含图片的消息。只有用户附件参与编号。已保存的位置标签会保留编号，包括删除草稿图片后留下的空号；其他附件按当前展示分支编号。成功的 Read 读图显示文件数量或字节摘要，其他工具和自定义消息的图片使用不带编号的 `[Image]` 链接。全屏模式点击附件行即可打开图片；普通模式使用终端的链接打开手势。支持的 PNG/JPEG/GIF/WebP 数据会异步生成私有临时文件，运行环境正常关闭时清理；缺失或不支持的数据保留不可用提示。主对话的原生搜索和提示导航也能定位附件行。

粘贴 PNG/JPEG/GIF/WebP 文件路径，或使用 Pi 原生的剪贴板图片操作，会在光标处插入 `[Image #N]` 标签。空格或换行分隔的绝对路径、家目录路径和文件 URL 会按顺序插入多个标签；列表内含空格的路径使用反斜杠转义。每个路径分别加引号的列表会保持原样。如果列表中至少有一张有效图片，缺失路径会被忽略，存在的非图片路径保留为文字；混入普通说明文字的粘贴内容保持原样。光标移动和删除把每个标签视为一个整体，Pi 原生撤销可以恢复删除的图片，也可以一次撤销整批粘贴。普通发送与后续消息队列通过 Pi 公开的输入转换接口传递当时捕获的图片字节和位置标签。点击草稿标签可打开对应图片；外部编辑器显示当前附件的可恢复标签；同一附件的多个引用只发送一份图片，删光引用才移除附件，分别粘贴的图片仍保持独立。关闭扩展时草稿保留可读的文件路径。手动输入路径仍是文本。历史取回和手动输入的 `[Image #N]` 文字标签也支持整体排版、移动、删除与撤销，发送时仍是文字，不会自动附加图片字节。草稿含有折叠粘贴内容时，文字标签删除暂时沿用 Pi 原生行为；取回队列或中断时，按 Pi 的即时插入、后续消息顺序恢复已观察到的图片草稿，同号文字标签仍保持文字。如果无法核对取回的队列内容，会保留文本并提示检查、重新附加缺失图片。

恢复会话后，图片编号会接在当前保存分支的用户文字标签之后。成功粘贴图片时也会参考当前可见草稿中的标签；丢弃的文字和本次运行中新发送的纯文字消息不消耗编号。已发送的图片通过 Pi 保存一份仅用于显示的编号记录，避免无关文字标签导致附件在发送、恢复后改号；记录不含源路径或图片字节，也不进入模型上下文。长文本粘贴目前仍使用 Pi 原生标签和独立计数。

扩展对话框关闭后，在同一个编辑器中恢复未修改的草稿，会保留折叠文本、图片附件、光标位置和原生撤销记录。此行为不负责把折叠粘贴数据转移给新建编辑器或修改后的草稿。

### 在 macOS 上粘贴图片

空输入时按 `?`，可查看 Pi 当前配置的图片粘贴快捷键，macOS 上通常是 **Control+V**。中文输入法的全角问号也可在空输入时打开帮助。本地图片可以通过以下两种方式添加：

- **复制图片内容：**在“预览”中打开图片，Command+A 选中图片，再按 Command+C。点击 Pi 输入框，按 Control+V，或自己配置的图片粘贴快捷键。
- **复制文件路径：**在 Finder 选中本地 PNG/JPEG/GIF/WebP 文件，按 Option+Command+C 复制完整路径。点击 Pi 输入框，使用终端的普通粘贴快捷键，例如 Orca 中的 Command+V。路径与问题文字分开粘贴。

确认出现 `[Image #N]` 标签后，再输入问题并发送。Command+V 由终端处理；复制文件名或文件引用，不一定会提供图片像素或可用路径。手动输入路径或 `[Image #N]` 不会添加图片。

| 现象 | 排查方法 |
| --- | --- |
| 两种粘贴方式都没有出现图片标签 | 确认图片已存在于运行 Pi 的这台 Mac。远程剪贴板软件可能只提供 0 字节占位文件，需要先完成真实文件传输。Finder 的“编辑 > 显示剪贴板”可查看是否为文件 URL；有文件 URL 并不代表图片内容已经可读。 |
| 已有图片标签，但模型无法识图 | 确认 provider 确实支持图片输入，并且 Pi 的 `models.json` 中当前自定义模型声明了 `"input": ["text", "image"]`。修改后打开原生 `/model` 刷新列表并重新选择模型。修改声明不能让纯文本接口获得视觉能力。 |
| 大照片发送耗时较长 | Pi-TUIX 捕获原始文件字节。原图较大时，可先缩小一份本地副本再粘贴。 |

可用 `/pituix-default` 切换到 Pi 默认输入框做对照，再用 `/pituix` 恢复界面。剪贴板读取、远程文件传输和模型能力仍由对应宿主负责。
...
在 Pi 0.84.4 中，原生 `/new`、`/resume` 或 `/reload` 之后可能恢复成 Pi 已保存的配色。Pi-TUIX 在 shell 启用期间观察公开的 `ctx.ui.theme`，发现宿主主题变化后会重新绑定自定义组件；停用 shell 后会停止观察，不修改 Pi 的私有主题控制器。此操作只临时切换主题，不修改 Pi 保存的主题设置；`/pituix-default` 会还原恢复操作之前的主题。如果之后又手动选择了其他主题，则保留该选择。
