...
使用 `pi --tui-mode fullscreen` 时，主界面会按参考样式实时显示用户和助手消息，同时保留 Pi 的编辑器、流式输出、工具、通知、组件和队列。Page Up/Down、Home/End、提示词跳转和垂直滚轮可以浏览对话。原生弹窗和全文搜索打开时会暂时显示 Pi 视图，关闭后恢复；搜索与主界面目前保留各自的滚动位置。普通终端模式仍使用 Pi 的消息样式。`/pituix-default` 可以在同一会话中恢复默认界面。
