...
工作行显示耗时和已上报的输出 token；每次运行完全结束后，通过 Pi 公开的自定义记录接口保存完成或中断提示。记录不进入模型上下文，默认界面隐藏其显示。

普通和全屏模式中的实时对话共享可恢复的消息样式，滚动、搜索、提示导航及编辑器仍由 Pi 管理。`/pituix-transcript` 打开当前分支的只读快照，显示 Markdown、工具、diff 和完成记录，Esc 返回编辑器。
