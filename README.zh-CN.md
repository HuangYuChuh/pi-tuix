...
主对话和这两种预览会在用户消息旁显示 `[Image #1]` 等图片附件，包括仅含图片的消息。只有用户附件参与编号，顺序遵循当前展示分支，重复图片也各自编号。成功的 Read 读图显示文件数量或字节摘要，其他工具和自定义消息的图片使用不带编号的 `[Image]` 链接。全屏模式点击附件行即可打开图片；普通模式使用终端的链接打开手势。支持的 PNG/JPEG/GIF/WebP 数据会异步生成私有临时文件，运行环境正常关闭时清理；缺失或不支持的数据保留不可用提示。主对话的原生搜索和提示导航也能定位附件行。图片粘贴和输入草稿仍沿用 Pi 原生行为；粘贴文件路径本身不会生成图片附件。
