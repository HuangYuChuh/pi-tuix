...
主对话和这两种预览会在用户消息旁显示 `[Image #1]` 等图片附件，包括仅含图片的消息。只有用户附件参与编号。已保存的位置标签会保留编号，包括删除草稿图片后留下的空号；其他附件按当前展示分支编号。成功的 Read 读图显示文件数量或字节摘要，其他工具和自定义消息的图片使用不带编号的 `[Image]` 链接。全屏模式点击附件行即可打开图片；普通模式使用终端的链接打开手势。支持的 PNG/JPEG/GIF/WebP 数据会异步生成私有临时文件，运行环境正常关闭时清理；缺失或不支持的数据保留不可用提示。主对话的原生搜索和提示导航也能定位附件行。粘贴单个 PNG/JPEG/GIF/WebP 文件路径，或使用 Pi 原生的剪贴板图片操作，会在光标处插入 `[Image #N]` 标签。光标移动和删除把标签视为一个整体，Pi 原生撤销也会恢复对应图片。普通发送与后续消息队列通过 Pi 公开的输入转换接口传递真实图片字节和位置标签。点击草稿标签可打开当时附加的图片；外部编辑器显示可恢复的标签，关闭扩展时草稿保留可读的文件路径。手动输入路径仍是文本。一次粘贴多个文件路径，以及在新运行环境中从历史输入重新附加图片，仍有差异。
