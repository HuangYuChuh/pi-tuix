...
const ANSI_APC = new RegExp(`${ESC}_[^${BEL}]*${BEL}`, "g");

export function stripAnsi(text: string): string {
  return text.replace(ANSI_SGR, "").replace(ANSI_OSC, "").replace(ANSI_APC, "");
}
...
export function sanitizeStatus(text: string): string {
  return Array.from(stripAnsi(text), (character) => {
...
export function pickSlashCommandTips(
  ...
  for (let i = pool.length - 1; i > 0; i--) {
    ...
    if (tmp === undefined || replacement === undefined) continue;
    pool[i] = replacement;
    pool[j] = tmp;
  }
