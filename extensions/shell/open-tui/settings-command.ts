...
const COPY = {
  en: {
    ...
    values: {
      ...
      wheelPrompt: (count: number) =>
        `Wheel scroll lines per notch, 1-10 (current: ${count}). Enter: apply · Esc: cancel`,
  ...
  zh: {
    ...
    values: {
      ...
      wheelPrompt: (count: number) =>
        `滚轮每格滚动行数（当前 ${count}，范围 1-10），输入后 Enter 应用 · Esc 取消`,
...
function setWheelScrollLines(config: OpenTuiConfig, raw: string): OpenTuiConfig | undefined {
  ...
  return {
    ...
    fullscreen: {
      ...config.fullscreen,
      wheelScrollLines: normalizeFullscreenWheelScrollLines(
        bounded,
        DEFAULT_FULLSCREEN_WHEEL_SCROLL_LINES,
      ),
    },
...
class SettingsUi implements SettingsUiHandle {
  ...
  private switchTab(offset: number): void {
    ...
    this.tab = TABS[(idx + offset + TABS.length) % TABS.length] ?? TABS[0];
    this.rebuild();
  }
  ...
  private rebuild(preferredItemId = this.selectedItemByTab[this.tab]): void {
    ...
    if (editingWheel) {
      const wheelInput = this.wheelInput;
      if (!wheelInput) return;
      wheelInput.focused = true;
      const wheelInputGroup = new Box(4, 0);
      wheelInputGroup.addChild(
        new Text(
      ...
      wheelInputGroup.addChild(wheelInput);
